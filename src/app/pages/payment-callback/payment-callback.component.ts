...
@Component({
  ...
  template: `
    <div class="min-h-screen flex items-center justify-center bg-[#0D0B18] p-4 font-sans">
      <div class="glass-card max-w-md w-full p-8 border border-[var(--bridge-border)] text-center rounded-3xl relative overflow-hidden shadow-2xl">
        <!-- Background Glow -->
        <div class="absolute -top-24 -left-24 w-48 h-48 bg-[#C62761]/20 rounded-full blur-3xl pointer-events-none"></div>
        <div class="absolute -bottom-24 -right-24 w-48 h-48 bg-[#F5A623]/20 rounded-full blur-3xl pointer-events-none"></div>
        ...
        <div *ngIf="!loading && success" class="space-y-6">
          <div class="w-20 h-20 rounded-3xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-4xl mx-auto text-emerald-400 animate-bounce">
            🎉
          </div>
          <div>
        ...
        <!-- State: Error / Fail -->
        <div *ngIf="!loading && !success" class="space-y-6">
          <div class="w-20 h-20 rounded-3xl bg-red-500/10 border border-red-500/30 flex items-center justify-center text-4xl mx-auto text-red-400">
            ⚠️
          </div>
          <div>
            <h2 class="text-2xl font-bold text-white font-syne">Paiement Non Validé</h2>
            <p class="text-sm text-red-300/80 mt-2">{{ errorMessage ? errorMessage : 'La transaction n\'a pas pu être confirmée par Stripe.' }}</p>
          </div>
