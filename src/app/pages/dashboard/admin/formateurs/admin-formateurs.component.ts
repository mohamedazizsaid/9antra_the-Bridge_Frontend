  ...
  template: `
    <div class="space-y-6 animate-fadeIn">
      <div class="flex items-center justify-between">
        ...
        <button (click)="showModal = true" class="bridge-btn-primary px-4 py-2.5 text-sm">
      ...
      <div class="bridge-card overflow-hidden">
            ...
            >
              <div class="flex items-center gap-3">
                <img
                  ...
                  alt=""
                  onerror="this.src='https://api.dicebear.com/7.x/initials/svg?seed='+f.firstName+f.lastName"
                />
                <div>
              ...
              <div class="flex items-center gap-3">
                ...
                <span class="text-xs text-[var(--bridge-text-muted)]">{{
                  f.dateInscription | date: 'dd/MM/yyyy'
                }}</span>
              </div>
            </div>
            <div
        ...
        <div class="px-5 py-3 border-t border-[var(--bridge-border)]">
          <button
            ...
            class="text-xs text-[var(--bridge-crimson)] hover:text-white transition-colors"
          >
      ...
      <div *ngIf="showModal" class="bridge-card overflow-hidden inline-view-card">
        ...
        >
          <div class="flex items-center gap-3">
            <div
              class="w-9 h-9 rounded-xl bg-gradient-to-br from-[#C62761] to-[#F5A623] flex items-center justify-center text-lg flex-shrink-0"
            >
              🎓
            </div>
            <div>
              <h3 class="font-syne font-bold text-white text-sm leading-tight">
        ...
        <div class="p-5 space-y-4">
          ...
          <!-- Notice -->
          <div
            class="flex items-start gap-3 bg-amber-500/[0.08] border border-amber-500/20 rounded-xl p-3.5"
          >
            <span class="text-lg flex-shrink-0">⚠️</span>
            <p class="text-xs text-amber-300/80 leading-relaxed">
              Un email sera envoyé au formateur avec un mot de passe temporaire.
            </p>
          </div>
          <!-- Feedback -->
          <p
            *ngIf="createError"
            class="text-red-400 text-xs text-center bg-red-500/10 border border-red-500/20 rounded-xl py-2.5"
          >
            {{ createError }}
          </p>
          <p
            *ngIf="createSuccess"
            class="text-emerald-400 text-xs text-center bg-emerald-500/10 border border-emerald-500/20 rounded-xl py-2.5"
          >
            {{ createSuccess }}
          </p>
        </div>
        ...
        <div class="flex gap-3 px-5 py-4 border-t border-[var(--bridge-border)]">
          <button
            (click)="showModal = false"
            class="flex-1 py-2.5 bg-white/5 hover:bg-white/10 text-white/70 hover:text-white font-semibold text-sm rounded-xl border border-white/5 hover:border-white/10 transition-all"
          >
            Annuler
          </button>
          <button
            ...
            [disabled]="
              creating || !newFormateur.firstName || !newFormateur.lastName || !newFormateur.email
            "
