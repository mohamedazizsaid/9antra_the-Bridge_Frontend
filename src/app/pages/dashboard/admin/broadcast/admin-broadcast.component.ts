    ...
    <div class="space-y-6 animate-fadeIn">
      <div>
        ...
        <p class="text-[var(--bridge-text-muted)] text-sm mt-1">
          Envoyer des notifications ciblées aux utilisateurs
        </p>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bridge-card p-6">
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                class="bridge-input w-full resize-none"
              ></textarea>
            </div>
            <div>
          ...
          <div *ngIf="successMsg" class="mt-3 text-center text-emerald-400 text-sm animate-fadeIn">
            {{ successMsg }}
          </div>
          ...
          <div class="bridge-card p-5">
            ...
            <div class="bg-[#08081A] rounded-xl p-4 border border-[var(--bridge-border)]">
              <div class="flex items-start gap-3">
                <div
                  class="w-10 h-10 rounded-xl bg-pink-500/10 flex items-center justify-center text-xl flex-shrink-0"
                >
                  📢
                </div>
                <div class="flex-1">
                  ...
                  <p class="text-xs text-[var(--bridge-text-muted)] mt-1">
                    {{ form.message || 'Contenu du message...' }}
                  </p>
            ...
            <div class="space-y-3">
              <div *ngFor="let h of history" class="p-3 bg-white/5 rounded-xl">
                <div class="flex items-center justify-between">
                  ...
                  <span
                    class="text-xs bg-emerald-500/10 text-emerald-400 px-2 py-0.5 rounded-full ml-2 flex-shrink-0"
                  >
                    {{ h.sent }} envoyés
                  </span>
                </div>
                <p class="text-xs text-[var(--bridge-text-muted)] mt-1 line-clamp-1">
                  {{ h.message }}
                </p>
                <div class="flex items-center gap-2 mt-2">
                  <span
                    *ngFor="let r of h.roles"
                    class="text-[9px] font-bold bg-white/10 text-white/60 px-1.5 py-0.5 rounded"
                    >{{ r }}</span
                  >
