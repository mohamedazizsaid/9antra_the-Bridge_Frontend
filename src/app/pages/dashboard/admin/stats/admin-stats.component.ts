  ...
  template: `
    <div class="space-y-6 animate-fadeIn">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 class="font-syne font-bold text-2xl text-white flex items-center gap-2">
            <span
              class="p-2 rounded-xl bg-gradient-to-br from-[#C62761]/20 to-[#F5A623]/20 border border-[#C62761]/30"
              >📈</span
            >
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            (click)="load()"
            class="bridge-btn-secondary px-4 py-2 text-xs flex items-center gap-2"
          >
            🔄 Actualiser
          </button>
        </div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div
          class="glass-card p-5 border border-[var(--bridge-border)] hover:border-blue-500/30 transition-all"
        >
          <div class="flex items-center justify-between mb-2">
            ...
            >
            <div class="w-8 h-8 rounded-lg bg-blue-500/10 flex items-center justify-center text-sm">
              👥
            </div>
          </div>
          ...
          <div class="flex items-center gap-2 mt-2 text-[10px]">
            <span class="text-emerald-400 font-semibold"
              >✓ {{ stats?.activeUsers || 0 }} actifs</span
            >
            <span class="text-white/30">•</span>
            <span class="text-[var(--bridge-text-muted)]"
              >{{ stats?.stagiaires || 0 }} stagiaires</span
            >
          </div>
        </div>

        <div
          class="glass-card p-5 border border-[var(--bridge-border)] hover:border-[#C62761]/30 transition-all"
        >
          <div class="flex items-center justify-between mb-2">
            <span
              class="text-xs text-[var(--bridge-text-muted)] uppercase tracking-wider font-semibold"
            ...
            >
            <div
              class="w-8 h-8 rounded-lg bg-[#C62761]/10 flex items-center justify-center text-sm"
            >
              💰
            </div>
          </div>
          ...
          <p class="text-[10px] text-emerald-400 font-semibold mt-2">
            Paiements validés Stripe & Espèces
          </p>
        </div>

        <div
          class="glass-card p-5 border border-[var(--bridge-border)] hover:border-[#F5A623]/30 transition-all"
        >
          <div class="flex items-center justify-between mb-2">
            <span
              class="text-xs text-[var(--bridge-text-muted)] uppercase tracking-wider font-semibold"
            ...
            >
            <div
              class="w-8 h-8 rounded-lg bg-[#F5A623]/10 flex items-center justify-center text-sm"
            >
              📚
            </div>
          </div>
          <p class="text-3xl font-mono font-bold text-[#F5A623]">
            {{ baseStats?.formations || 0 }}
          </p>
          <p class="text-[10px] text-[var(--bridge-text-muted)] mt-2">
            {{ baseStats?.evaluations || 0 }} évaluations réalisées
          </p>
        </div>

        <div
          class="glass-card p-5 border border-[var(--bridge-border)] hover:border-purple-500/30 transition-all"
        >
          <div class="flex items-center justify-between mb-2">
            <span
              class="text-xs text-[var(--bridge-text-muted)] uppercase tracking-wider font-semibold"
            ...
            >
            <div
              class="w-8 h-8 rounded-lg bg-purple-500/10 flex items-center justify-center text-sm"
            >
              🏆
            </div>
          </div>
          <p class="text-3xl font-mono font-bold text-purple-400">
            {{ baseStats?.certificates || 0 }}
          </p>
