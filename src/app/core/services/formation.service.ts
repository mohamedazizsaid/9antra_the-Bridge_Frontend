...
export class FormationService {
  ...
  private mapFormationDTO(f: any): Formation {
    ...
    const studentIds: string[] = rawStudents
      .map((s: any) => (typeof s === 'object' ? s.id?.toString() || '' : s?.toString() || ''))
      .filter((id: string) => id !== '');
    ...
    return {
      ...
      formateurNom: mainTrainer
        ? `${mainTrainer.firstName} ${mainTrainer.lastName}`
        : 'Aucun formateur',
  ...
  getFormations(): Observable<Formation[]> {
    return this.http
      .get<any[]>(`${this.apiUrl}/formations`)
      .pipe(map((list) => list.map((f) => this.mapFormationDTO(f))));
  }

  getFormationById(id: string): Observable<Formation | undefined> {
    return this.http
      .get<any>(`${this.apiUrl}/formations/${id}`)
      .pipe(map((f) => this.mapFormationDTO(f)));
  }

  getFormationsByFormateur(formateurId: string): Observable<Formation[]> {
    return this.http
      .get<any[]>(`${this.apiUrl}/formations/formateur/${formateurId}`)
      .pipe(map((list) => list.map((f) => this.mapFormationDTO(f))));
  }

  getFormationsByStagiaire(stagiaireId: string): Observable<Formation[]> {
    return this.http
      .get<any[]>(`${this.apiUrl}/formations/stagiaire/${stagiaireId}`)
      .pipe(map((list) => list.map((f) => this.mapFormationDTO(f))));
  }
  ...
  getUpcomingSeances(trainerId?: string): Observable<Seance[]> {
    ...
    return this.http.get<any[]>(url).pipe(map((list) => list.map((s) => this.mapSessionDTO(s))));
  }
  ...
  savePresence(seanceId: string, presences: Presence[]): Observable<boolean> {
    ...
    return this.http
      .post(`${this.apiUrl}/attendance/session/${seanceId}`, payload)
      .pipe(map(() => true));
  }

  createFormation(formation: any): Observable<Formation> {
    return this.http
      .post<any>(`${this.apiUrl}/formations`, formation)
      .pipe(map((f) => this.mapFormationDTO(f)));
  }
  ...
  assignTrainers(formationId: string, trainerIds: number[]): Observable<Formation> {
    return this.http
      .put<any>(`${this.apiUrl}/formations/${formationId}/trainers`, trainerIds)
      .pipe(map((f) => this.mapFormationDTO(f)));
  }
  ...
  updateFormation(id: string, data: Partial<Formation>): Observable<Formation> {
    ...
    return this.http
      .put<any>(`${this.apiUrl}/formations/${id}`, payload)
      .pipe(map((f) => this.mapFormationDTO(f)));
  }
