...
@Component({
  ...
  template: `
    ...
    >
      <!-- Glow effect -->
      <div
        class="absolute inset-0 bg-gradient-to-br from-pink-500/10 to-amber-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
      ></div>

      <div class="flex items-center justify-between">
        <div>
          <span
            class="text-sm font-medium text-[var(--bridge-text-muted)] uppercase tracking-wider"
            >{{ label }}</span
          >
      ...
      <div *ngIf="trend" class="mt-4 flex items-center gap-1.5 text-xs">
        <span
          [ngClass]="trendPositive ? 'text-emerald-400' : 'text-rose-400'"
          class="font-semibold"
        >
          {{ trendPositive ? '↑' : '↓' }} {{ trend }}
        </span>
